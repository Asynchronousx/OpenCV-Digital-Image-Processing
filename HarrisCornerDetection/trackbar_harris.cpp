...
void harris(Mat, Mat, Mat&); 
void slideHarris(int , void*);

int  max_upper_t = 10000;
int upper_t;
...
//Public Mat objects for the trackbar function
Mat public_DX;
Mat public_DY;
Mat public_padded;
Mat public_dest;
...
int main(int argc, char** argv) {
	...
	char trackbar_name[50];
 	sprintf(trackbar_name, "Threesold: %d", upper_t);
	createTrackbar(trackbar_name, "Harris", &upper_t, max_upper_t, slideHarris );
	
	//call the harris functions
	harris(raw_image, kernel, dest_image);
	slideHarris(upper_t, 0);
...
void harris(Mat raw_image, Mat kernel, Mat& dest_image) {
	...
	Mat padded_image;
	padding(raw_image, padded_image, 3);
	dest_image = raw_image.clone();
	
	public_DX = DX;
	public_DY = DY;
	public_padded = padded_image;
...
void slideHarris(int , void*) {
	...
	for(int i=pad; i<public_padded.rows-pad; i++) {
		for(int j=pad; j<public_padded.cols-pad; j++) {
	...
	//draw some shape to identify this corner
	for(int i=0; i<corners.size(); i++) {
		...
		circle(public_dest, center, 5, Scalar(0,255,0), 1, 4, 0);
	}	
	
	imshow("Harris", public_dest);
