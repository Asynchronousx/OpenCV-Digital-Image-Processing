...
int main(int argc, char** argv) {
	...
	imwrite("PrewittMult"+name, dest_image);
	waitKey(0);
	return(0);
...
void PrewittOperator(Mat raw_image, Mat& dest_image) {
	...
	for(int i=0; i<raw_image.rows; i++) {
		for(int j=0; j<raw_image.cols; j++) {
			gradientX = ((-1*raw_image.at<uchar>(i,j)) + (-1*raw_image.at<uchar>(i+1,j)) + (-1*raw_image.at<uchar>(i+2,j))) + 
						((1*raw_image.at<uchar>(i,j+2)) + (1*raw_image.at<uchar>(i+1,j+2)) + (1*raw_image.at<uchar>(i+2,j+2)));
