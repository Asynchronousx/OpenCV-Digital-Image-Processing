...
void grow(vector<Point>, Mat, Mat&, Mat&, Vec3b);
...
int main(int argc, char** argv) {
	...
	Mat dest_image(raw_image.size(), raw_image.type(), Scalar(0, 0, 0));
...
void RegionGrowing(Mat raw_image, Mat& regions, Mat& dest_image) {
	...
	//Init a seed for the x and y position
	int x_seed, y_seed;
	...
	for(int i=0; i<quality; i++) {
		...
		if(regions.at<uchar>(x_seed, y_seed) == 0) {
			...
			//pixel with similar euclidean distance to this one, that represent the first seed of the current region.
			//Note: since we're operating into the color space, we need to access the image matrix at a Vec3b type and not the uchar one.
			...
			//Same note as above
			dest_image.at<Vec3b>(x_seed, y_seed) = pixel_value;		
...
void grow(vector<Point> pixelVector, Mat raw, Mat& regions, Mat& dest, Vec3b pixel_value) {		
		...
		for(int u=-1; u<=1; u++) {
			for(int v=-1; v<=1; v++) {
				...
				if(x+u >=0 && x+u < raw.rows && y+v >=0 && y+v < raw.cols) {
					...
					//Calculating if a pixel should belong to a certain region is far easier into the color space: we just need to calculate the
					//euclidean pixel between the pixel passed in input (generated from the seed x and y coordinate) and the current pixel analyzed 
					//from the sliding window. 
					//The euclidean distance, given the two ternary couple (x,y,z) that represent the current pixel, and (a,b,c) that represent the seed
					//of the region, is calculated by:
					//SQRT((x-a)^2 + (y-b)^2 + (z-c)^2). x, y and z represent the Blue, Green and Red value of the current pixel analyzed, while
					//a, b, c represent also the B G R values of the seed pixel passed to that function.
					//If the value is BIGGER than the threshold, we know for sure that the pixel should not belong to that region because differs too much.
					//We also check that the current pixel has not been analyzed yet (region(current pixel) == 0)
					
					//Calculate the distance with the euclidean formula
					distance = sqrt(pow(raw.at<Vec3b>(x+u, y+v).val[0] - pixel_value.val[0], 2) + 
									pow(raw.at<Vec3b>(x+u, y+v).val[1] - pixel_value.val[1], 2) +
									pow(raw.at<Vec3b>(x+u, y+v).val[2] - pixel_value.val[2], 2));
									
					//the distance could computed also with: distance = norm(pixel_value, raw.at<Vec3b>(x+u, y+v));
					
					if(distance < t && regions.at<uchar>(x+u, y+v) == 0) {
						...
						dest.at<Vec3b>(x+u, y+v) = pixel_value;	
