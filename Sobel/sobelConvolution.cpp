...
Mat prewitt_vertical = (Mat_<char>(3,3) <<  1,  2,  1, 
										    0,  0,  0,
										   -1, -2, -1);

void padding(Mat, Mat&);
void SobelOperator(Mat, Mat, Mat&);
...
int main(int argc, char** argv) {
	...
	SobelOperator(raw_image, padded_image, dest_image);

	string name(argv[1]);
	
	imshow("Colored Image", original_image);
	imshow("Original Image", raw_image);
	imshow("Sobel Image", dest_image);
	imwrite("SobelConv"+name, dest_image);
	waitKey(0);
	return(0);
...
void SobelOperator(Mat raw_image, Mat padded_image, Mat& dest_image) {
